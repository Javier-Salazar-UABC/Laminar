...
* **Panel de Filtros Interactivos:** Habilitado el panel de filtros debajo de la barra de navegación para permitir búsquedas cruzadas por extensión/tipo de archivo, estado de Git (`modified`, `new`, `uptodate`) y etiquetas asociadas. *(Corrección de UI: se removió `backdrop-filter` y el cálculo de altura `h-full` de la lista vacía para solventar un bug de repintado infinito en Chromium/QWebEngine que expandía la pantalla con un recuadro blanco al seleccionar opciones).*
...
* **Iconografía de Lenguajes de Programación:** Integración de la librería profesional `react-icons/si` (Simple Icons) para renderizar los logotipos oficiales, exactos y pixel-perfect de cada tecnología (Python, JavaScript, TypeScript, React, HTML5, CSS3, JSON y Markdown) con sus respectivos colores corporativos, garantizando coherencia visual y calidad premium.
* **Barra Superior Altamente Responsive (Toolbar):** Rediseñada la estructura flex de la barra superior. Se limitó el ancho del breadcrumb de forma adaptativa para que colapse con elipsis en rutas de archivo muy largas, y se fijó la botonera de acciones del extremo derecho (`shrink-0` con búsqueda y espaciado adaptativo). Esto asegura que botones esenciales (como el selector de filtros y el botón de cambiar de tema) permanezcan perfectamente encuadrados y visibles sin desbordar la pantalla en ventanas estrechas.
